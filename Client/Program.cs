...
Console.WriteLine("1 - Использовать инструменты (создать новую базу)");
Console.WriteLine("2 - Продолжить и выбрать существующую базу");
...
if (mode == 1)
{
    ...
    // После создания сканируем папку заново
    string[] dbFiles = Directory.Exists(dbFolder) ? Directory.GetFiles(dbFolder, "*.db") : new string[0];
    if (dbFiles.Length == 0)
    {
        Console.WriteLine("Нет доступных баз после создания");
        return;
    }

    selectedDb = dbFiles[^1]; // берем последнюю созданную базу
}
...
else if (mode == 2)
{
    ...
    selectedDb = dbFiles[selectedIndex];
}
